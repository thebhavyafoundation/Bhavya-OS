...
### 7. Design Workflows (SPDD Integration)

- `design.reasons_canvas`: Requirements capture → Repository inspection → REASONS Canvas generation → Human review → Save to `specs/`.
- `design.drift_check`: Compare REASONS Canvas against actual implementation → Report MATCH/DRIFT per item → Investigate discrepancies.
