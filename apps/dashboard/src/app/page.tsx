    ...
    <div style={{ maxWidth: 1400, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 16, color: "#94a3b8", maxWidth: 640 }}>
          Real-time view of institutional health, mission activity, and knowledge growth.
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 32 }}>
        ...
        <WidgetCard title="Mission Activity">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 4px 0" }}>Forest</p>
              <p style={{ fontSize: 20, fontWeight: 600, color: "#10b981", margin: 0 }}>{forestMissions} missions</p>
            </div>
            <div>
              <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 4px 0" }}>Heritage</p>
              <p style={{ fontSize: 20, fontWeight: 600, color: "#f59e0b", margin: 0 }}>{heritageMissions} missions</p>
            </div>
            <div>
              <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 4px 0" }}>Research</p>
              <p style={{ fontSize: 20, fontWeight: 600, color: "#ec4899", margin: 0 }}>{researchProjects} projects</p>
            </div>
            <div>
              <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 4px 0" }}>Volunteers</p>
              <p style={{ fontSize: 20, fontWeight: 600, color: "#06b6d4", margin: 0 }}>{volunteerCount} people</p>
            </div>
          </div>
        </WidgetCard>
        ...
        <WidgetCard title="Knowledge Health">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 4px 0" }}>Published</p>
              <p style={{ fontSize: 20, fontWeight: 600, color: "#10b981", margin: 0 }}>{publishedDocs}</p>
            </div>
            <div>
              <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 4px 0" }}>Drafts</p>
              <p style={{ fontSize: 20, fontWeight: 600, color: "#f59e0b", margin: 0 }}>{draftDocs}</p>
            </div>
            <div>
              <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 4px 0" }}>Coverage</p>
              <p style={{ fontSize: 20, fontWeight: 600, color: "#8b5cf6", margin: 0 }}>
                {docs.length > 0 ? Math.round((publishedDocs / docs.length) * 100) : 0}%
              </p>
            </div>
            <div>
              <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 4px 0" }}>Graph Density</p>
              <p style={{ fontSize: 20, fontWeight: 600, color: "#3b82f6", margin: 0 }}>
                {kg.length > 0 ? Math.round((totalEdges / kg.length) * 100) / 100 : 0}
              </p>
            </div>
          </div>
        </WidgetCard>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 24, marginBottom: 32 }}>
        ...
        <WidgetCard title="Recent Publications">
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {recentDocs.map((doc) => (
              <div key={doc.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #334155" }}>
                <div>
                  <p style={{ fontSize: 14, fontWeight: 500, color: "#f8fafc", margin: 0 }}>{doc.title}</p>
                  <p style={{ fontSize: 12, color: "#64748b", margin: "2px 0 0 0" }}>{doc.category}</p>
                </div>
                <span style={{ fontSize: 11, color: doc.status === "published" ? "#10b981" : "#f59e0b" }}>
                  {doc.status}
                </span>
              </div>
            ))}
            ...
            {Object.entries(docsByCategory)
              ...
              .map(([category, count]) => (
                <div key={category} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <p style={{ fontSize: 13, color: "#94a3b8", margin: 0, textTransform: "capitalize" }}>{category}</p>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 120, height: 6, background: "#334155", borderRadius: 3 }}>
                      <div style={{ width: `${(count / docs.length) * 100}%`, height: "100%", background: "#10b981", borderRadius: 3 }} />
                    </div>
                    <p style={{ fontSize: 13, fontWeight: 600, color: "#f8fafc", margin: 0, minWidth: 24, textAlign: "right" }}>{count}</p>
                  </div>
                </div>
              ))}
          </div>
        ...
        <WidgetCard title="Platform KPIs">
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              { label: "Publication Rate", value: `${docs.length > 0 ? Math.round((publishedDocs / docs.length) * 100) : 0}%`, color: "#10b981" },
              { label: "Graph Density", value: kg.length > 0 ? (totalEdges / kg.length).toFixed(1) : "0", color: "#3b82f6" },
              { label: "Entity Coverage", value: `${entities.length} entities`, color: "#8b5cf6" },
              { label: "Validation", value: "PASS", color: "#10b981" },
              { label: "Tests", value: "55/55", color: "#10b981" },
              { label: "Missions Active", value: `${forestMissions + heritageMissions + researchProjects}`, color: "#f59e0b" },
            ].map((kpi) => (
              <div key={kpi.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>{kpi.label}</p>
                <p style={{ fontSize: 13, fontWeight: 600, color: kpi.color, margin: 0 }}>{kpi.value}</p>
              </div>
            ))}
          </div>
        </WidgetCard>
            ...
            <div key={insight.id} style={{ padding: "12px 16px", background: "#0f172a", borderRadius: 8, border: "1px solid #334155" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                ...
                <span style={{
                  ...
                  padding: "2px 8px",
                  borderRadius: 4,
                  background: insight.type === "publication" ? "#10b98120" : insight.type === "entity" ? "#8b5cf620" : insight.type === "mission" ? "#3b82f620" : "#f59e0b20",
                  color: insight.type === "publication" ? "#10b981" : insight.type === "entity" ? "#8b5cf6" : insight.type === "mission" ? "#3b82f6" : "#f59e0b",
                }}>
                  {insight.type}
                </span>
              </div>
              <p style={{ fontSize: 12, color: "#94a3b8", margin: 0 }}>{insight.description}</p>
              <p style={{ fontSize: 11, color: "#64748b", margin: "4px 0 0 0" }}>{insight.source}</p>
            </div>
          ))}
        </div>
      </WidgetCard>
