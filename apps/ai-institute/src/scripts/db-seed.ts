...
import { migrate } from "../lib/sqlite";
...
async function main() {
  ...
  insertUser.run(adminId, "admin@ai-institute.com", "Platform Admin", adminHash, "admin", '["platform","education"]', now, now);
  insertUser.run(instructorId, "instructor@ai-institute.com", "Dr. Bhavya", adminHash, "instructor", '["ai","education","ethics"]', now, now);
  insertUser.run(student1Id, "student@ai-institute.com", "Demo Student", studentHash, "student", '["ai","web"]', now, now);
  insertUser.run(student2Id, "researcher@ai-institute.com", "Demo Researcher", studentHash, "researcher", '["ai","research"]', now, now);
  ...
  db.prepare(`INSERT INTO sessions (user_id, token, expires_at, created_at) VALUES (?, ?, ?, ?)`).run(
    student1Id, sessionToken, new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), now
  );
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, 'active', ?, ?)
  `).run(courseId, "AI Foundations", "Complete AI learning path from basics to deployment", instructorId, now, now);
  ...
  for (const lesson of lessons) {
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(uuid(), courseId, lesson.title, `Content for ${lesson.title}`, lesson.idx, now, now);
  }
  ...
  insertStudent.run(uuid(), student1Id, "Demo Student", "student@ai-institute.com", "student",
    '["ai","web"]', courseId, 2, '["lesson-1","lesson-2"]', 75, 60, 5, now,
    `["${courseId}"]`, now, now);

  insertStudent.run(uuid(), student2Id, "Demo Researcher", "researcher@ai-institute.com", "researcher",
    '["ai","research"]', courseId, 0, '[]', 0, 0, 0, now,
    `["${courseId}"]`, now, now);

  console.log("✓ Created 2 student profiles");

  // ─── Progress Records ─────────────────────────────────────────────────────
  const lessonIds = db.prepare("SELECT id FROM lessons WHERE course_id = ? ORDER BY order_index").all(courseId) as { id: string }[];
