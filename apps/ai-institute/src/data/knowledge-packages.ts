/**
 * Bhavya Academy — Knowledge Packages
 *
 * Structured educational packages with artifacts and learning outcomes.
 * Migrated from apps/website/src/data/knowledge-packages.ts to canonical host.
 *
 * @module knowledge-packages
 */
...
export function getKnowledgePackageById(
  id: string,
): KnowledgePackage | undefined {
  return knowledgePackages.find((kp) => kp.id === id);
}

export function getKnowledgePackagesByLevel(level: number): KnowledgePackage[] {
  return knowledgePackages.filter((kp) => kp.level === level);
}
