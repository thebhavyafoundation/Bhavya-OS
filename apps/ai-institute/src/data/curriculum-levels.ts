/**
 * Bhavya Academy — Curriculum Levels
 *
 * 13 progressive levels from Digital Foundations to Institution Building.
 * Migrated from apps/website/src/data/curriculum.ts to canonical host.
 *
...
export function getLevelByNumber(level: number): CurriculumLevel | undefined {
  return curriculum.find((l) => l.level === level);
}

export function getTotalModules(): number {
  return curriculum.reduce((sum, l) => sum + l.moduleCount, 0);
}
