...
import {
  ...
  createCommunityRequest, addStudentFeedback,
...
export async function POST(request: NextRequest) {
  ...
  switch (action) {
    case 'create-kp': {
      ...
      const kp = createKP({ title, level, domain, assignee, dueDate, concepts });
      return NextResponse.json({ kp }, { status: 201 });
    }
    case 'update-status': {
      const { kpId, status, stage } = body;
      if (!kpId || !status) return NextResponse.json({ error: 'kpId and status required' }, { status: 400 });
      const kp = updateKPStatus(kpId, status as KPStatus, stage as ProductionStage | undefined);
      return NextResponse.json({ kp });
    }
    case 'advance': {
      const { kpId } = body;
      if (!kpId) return NextResponse.json({ error: 'kpId required' }, { status: 400 });
      const kp = advanceKP(kpId);
      return NextResponse.json({ kp });
    }
    case 'create-media': {
      const { kpId, type, title, platform } = body;
      if (!kpId || !type || !title) return NextResponse.json({ error: 'kpId, type, and title required' }, { status: 400 });
      const asset = createMediaAsset({ kpId, type: type as MediaAssetType, title, platform });
      return NextResponse.json({ asset }, { status: 201 });
    }
    case 'community-request': {
      const { title, description, requestedBy } = body;
      if (!title) return NextResponse.json({ error: 'title required' }, { status: 400 });
      const request = createCommunityRequest({ title, description, requestedBy });
      return NextResponse.json({ request }, { status: 201 });
    }
    case 'student-feedback': {
      const { kpId, score, comment } = body;
      if (!kpId || score === undefined) return NextResponse.json({ error: 'kpId and score required' }, { status: 400 });
