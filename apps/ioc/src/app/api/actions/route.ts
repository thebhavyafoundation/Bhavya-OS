...
import { createActionItem, listActionItems, updateActionStatus, completeAction, getActionSummary } from '@/lib/actions';
...
export async function GET(request: NextRequest) {
  ...
  switch (action) {
    ...
    default: {
      ...
      const priority = searchParams.get('priority') as ActionPriority | undefined;
...
export async function POST(request: NextRequest) {
  ...
  switch (action) {
    case 'create': {
      const { title, description, type, priority, owner, dueDate, linkedRiskId, linkedObjectiveId } = body;
      if (!title || !type) return NextResponse.json({ error: 'title and type required' }, { status: 400 });
      const item = createActionItem({ title, description, type, priority, owner, dueDate, linkedRiskId, linkedObjectiveId });
