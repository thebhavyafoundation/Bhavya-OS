  ...
  if (loading)
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-sage">Loading reviews...</div>
      </div>
    );
    ...
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sage mt-2">
              Automated executive summaries and next-week plans
            </p>
          </div>
          <button
            onClick={generate}
        ...
        <div className="space-y-6">
          {reviews.map((review) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs ${review.status === "approved" ? "bg-green-900 text-green-300" : "bg-gray-800 text-gray-300"}`}
                >
                  {review.status}
                </span>
              </div>
          ...
          {reviews.length === 0 && (
            <div className="text-center text-sage py-12">
              No reviews yet. Click &quot;Generate Review&quot; to create one.
            </div>
          )}
        </div>
      </div>
