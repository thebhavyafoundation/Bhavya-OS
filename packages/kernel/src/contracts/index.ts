...
export function validateAgent(input: unknown): ContractResult {
  ...
  if (!agent.id || typeof agent.id !== 'string') errors.push('Agent id is required and must be a string');
  if (!agent.name || typeof agent.name !== 'string') errors.push('Agent name is required and must be a string');
  if (!agent.role || typeof agent.role !== 'string') errors.push('Agent role is required and must be a string');
  if (!Array.isArray(agent.capabilities)) errors.push('Agent capabilities must be an array');
...
export function validateWorkflow(input: unknown): ContractResult {
  ...
  if (!workflow.id || typeof workflow.id !== 'string') errors.push('Workflow id is required');
  if (!workflow.name || typeof workflow.name !== 'string') errors.push('Workflow name is required');
...
export function validateEvent(input: unknown): ContractResult {
  ...
  if (!event.source || typeof event.source !== 'string') errors.push('Event source is required');
  if (!event.payload || typeof event.payload !== 'object') errors.push('Event payload must be an object');
...
export function validateMemory(input: unknown): ContractResult {
  ...
  if (!memory.id || typeof memory.id !== 'string') errors.push('Memory id is required');
  if (!memory.type || typeof memory.type !== 'string') errors.push('Memory type is required');
  if (!memory.content || typeof memory.content !== 'string') errors.push('Memory content is required');
  if (!Array.isArray(memory.tags)) errors.push('Memory tags must be an array');

  const validTypes = ['project', 'person', 'knowledge', 'architecture', 'history', 'bug', 'lesson'];
...
export function validateGoal(input: unknown): ContractResult {
  ...
  if (!goal.id || typeof goal.id !== 'string') errors.push('Goal id is required');
  if (!goal.description || typeof goal.description !== 'string') errors.push('Goal description is required');
  if (goal.description && (goal.description as string).length < 10) errors.push('Goal description must be at least 10 characters');
  ...
  if (!Array.isArray(goal.constraints)) errors.push('Goal constraints must be an array');
...
export function validateTask(input: unknown): ContractResult {
  ...
  if (!task.type || typeof task.type !== 'string') errors.push('Task type is required');
  if (!task.goal || typeof task.goal !== 'string') errors.push('Task goal is required');
  if (!task.input || typeof task.input !== 'object') errors.push('Task input must be an object');
...
export function validatePlan(input: unknown): ContractResult {
  ...
  if (!plan.goalId || typeof plan.goalId !== 'string') errors.push('Plan goalId is required');
  if (!Array.isArray(plan.steps)) errors.push('Plan steps must be an array');

  const validStatuses = ['draft', 'approved', 'executing', 'completed', 'failed'];
