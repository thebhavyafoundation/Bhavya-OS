...
import { WebsiteContentService } from '../services/website-content.js';
...
export class IntegrationTest {
  ...
  async runAll(): Promise<{ passed: number; failed: number; total: number; results: TestResult[] }> {
    ...
    await this.test('Production Configuration', async () => {
      ...
      if (config.get('environment') !== 'development') throw new Error('Wrong environment');
    });

    await this.test('Event Bus', async () => {
      const events = new EventBus();
      let received = false;
      events.on('test', async () => { received = true; });
    ...
    await this.test('Memory Engine', async () => {
      ...
      await memory.set({ type: 'test', content: 'hello', tags: [], source: 'test', confidence: 1 });
    ...
    await this.test('Auth', async () => {
      const auth = new Auth({ secret: 'a'.repeat(32), tokenExpiryMs: 3600, refreshExpiryMs: 86400, issuer: 'test' });
    ...
    await this.test('Monitoring', async () => {
      const monitoring = new Monitoring({ alertThresholds: { errors: 10 }, sampleRate: 1, retentionMs: 3600000 });
    ...
    await this.test('Backup & Recovery', async () => {
      const backup = new BackupRecovery({ root: process.cwd(), backupDir: '/tmp/bhavya-backups', maxBackups: 3, autoBackupIntervalMs: 0 });
    ...
    await this.test('Self-Organizing Engine', async () => {
      const engine = new SelfOrganizingEngine({
        ...
        memory: { set: async () => ({}), get: async () => undefined, search: async () => [] },
        agents: { getAll: () => [], getAvailable: () => [], getBestMatch: () => undefined },
      });
      await engine.initialize();
      const work = await engine.discoverWork({ type: 'task', title: 'Test', description: 'Test', requiredCapabilities: ['test'] });
    ...
    await this.test('Consensus Engine', async () => {
      ...
      await engine.initialize();
      const proposal = await engine.createProposal({ title: 'Test', description: 'Test', proposedBy: 'a1', type: 'decision' });
      if (!proposal.id) throw new Error('No proposal');
    });

    await this.test('Capability Matcher', async () => {
      const matcher = new CapabilityMatcher({
        agents: { getAll: () => [{ id: 'a1', name: 'Agent 1', capabilities: ['react'], availability: 'available', currentWorkload: 0, maxWorkload: 5, performance: 0.9 }] },
      });
      await matcher.initialize();
      const result = await matcher.match({ id: 'req-1', requiredCapabilities: ['react'], priority: 'high' });
      if (!result.bestMatch) throw new Error('No match');
    });

    await this.test('Website Content Service', async () => {
      const service = new WebsiteContentService(process.cwd());
