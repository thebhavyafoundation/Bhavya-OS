...
export class EndToEndTest {
  ...
  async runAll(): Promise<WorkflowResult[]> {
    ...
    await this.workflow('New Page Creation', async () => {
    ...
    await this.workflow('Volunteer Onboarding', async () => {
      ...
      // Simulate volunteer workflow
      const artifacts: string[] = [];
      artifacts.push('volunteers/vol:123.json');
    ...
    await this.workflow('Multi-Agent Consensus', async () => {
      ...
      await engine.castVote(proposal.id, { agentId: 'a1', decision: 'approve', reasoning: 'Best practice' });
      await engine.castVote(proposal.id, { agentId: 'a2', decision: 'approve', reasoning: 'Agree' });
  ...
  private async workflow(name: string, fn: () => Promise<string[]>): Promise<void> {
    const start = Date.now();
    try {
      const artifacts = await fn();
      const duration = Date.now() - start;
      this.results.push({ workflow: name, steps: artifacts.length, artifacts, duration, success: true });
      console.log(`  ✓ ${name} (${duration}ms) — ${artifacts.length} artifacts`);
    } catch (err) {
      const duration = Date.now() - start;
      this.results.push({ workflow: name, steps: 0, artifacts: [], duration, success: false });
