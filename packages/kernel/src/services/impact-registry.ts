...
export interface ImpactInput {
  action: 'record-metric' | 'generate-report' | 'get-report' | 'get-reports' | 'get-summary' | 'compare-missions';
  missionId?: string;
  metricId?: string;
  value?: number;
  notes?: string;
...
export class ImpactRegistry {
  ...
  async execute(input: ImpactInput): Promise<{ success: boolean; result?: any }> {
    switch (input.action) {
      case 'record-metric':
  ...
  private async recordMetric(input: ImpactInput): Promise<{ success: boolean }> {
    if (!input.missionId || !input.metricId || input.value === undefined) return { success: false };
  ...
  private async generateReport(input: ImpactInput): Promise<{ success: boolean; report?: ImpactReport }> {
    ...
    this.audit('generate-report', input.missionId, 'impact-registry', { period: input.period });

    return { success: true, report };
  }

  private async getReport(input: ImpactInput): Promise<{ success: boolean; report?: ImpactReport }> {
    if (!input.reportId) return { success: false };
    const report = this.reports.get(input.reportId);
    return { success: !!report, report };
  }

  private async getReports(input: ImpactInput): Promise<{ success: boolean; reports: ImpactReport[] }> {
    if (!input.missionId) return { success: false, reports: [] };
    const reports = Array.from(this.reports.values()).filter((r) => r.missionId === input.missionId);
    return { success: true, reports };
  }

  private async getSummary(input: ImpactInput): Promise<{ success: boolean; summary: Record<string, unknown> }> {
  ...
  private async compareMissions(input: ImpactInput): Promise<{ success: boolean; comparison: Record<string, unknown> }> {
