...
export class RecognitionService {
  ...
  private async awardHours(
    input: RecognitionInput,
  ): Promise<{ success: boolean; recognition?: Recognition }> {
    if (!input.personId || !input.hours || !input.missionId)
    ...
    this.audit("award-hours", recognition.id, "recognition", {
      hours: input.hours,
    });
  ...
  private async awardAchievement(
    ...
    this.recognitions.set(recognition.id, recognition);
    this.audit("award-achievement", recognition.id, "recognition", {
  ...
  private async awardCertificate(
    ...
    this.recognitions.set(recognition.id, recognition);
    this.audit("award-certificate", recognition.id, "recognition", {
      cert: input.certName,
    });
  ...
  private async addRecommendation(
    ...
    if (!input.personId || !input.recommendedBy || !input.description)
      return { success: false };
    ...
    this.recognitions.set(recognition.id, recognition);
    this.audit("add-recommendation", recognition.id, "recognition", {
      by: input.recommendedBy,
    });
  ...
  private async getByPerson(
    input: RecognitionInput,
  ): Promise<{ success: boolean; recognitions: Recognition[] }> {
    if (!input.personId) return { success: false, recognitions: [] };
    const recognitions = Array.from(this.recognitions.values()).filter(
      (r) => r.personId === input.personId,
    );
  ...
  private async getStats(
    ...
    const personRecs = Array.from(this.recognitions.values()).filter(
      (r) => r.personId === input.personId,
    );
    const totalHours = personRecs
      .filter((r) => r.type === "hours")
      .reduce((sum, r) => sum + (r.value ?? 0), 0);
    const achievements = personRecs.filter(
      (r) => r.type === "achievement",
    ).length;
    const certificates = personRecs.filter(
