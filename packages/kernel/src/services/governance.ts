...
export class GovernanceService implements InstitutionService {
  ...
  private async create(
    ...
    artifacts.push({
      path: `docs/governance/active/${filename}`,
      action: "created",
      content,
      metadata: {},
    });
  ...
  private async archive(
    ...
    const content = readFileSync(srcPath, "utf-8").replace(
      "**Status:** Active",
      "**Status:** Archived",
    );
    ...
    artifacts.push({
      path: `docs/governance/archived/${filename}`,
      action: "created",
      content,
      metadata: {},
    });
