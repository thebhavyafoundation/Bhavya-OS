...
export class ResearchService implements InstitutionService {
  ...
  async initialize(): Promise<void> {
    ...
    if (existsSync(researchDir)) {
      const files = readdirSync(researchDir).filter((f: string) =>
        f.endsWith(".md"),
      );
  ...
  async execute(
    ...
    artifacts: Artifact[];
    success: boolean;
    results?: Array<{ title: string; content: string; topic: string }>;
  }> {
  ...
  private async summarize(
    input: ResearchInput,
    artifacts: Artifact[],
  ): Promise<{ artifacts: Artifact[]; success: boolean; summary: string }> {
