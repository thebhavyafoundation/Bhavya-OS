...
export class TransparencyService implements InstitutionService {
  ...
  private async publishFinancials(
    input: TransparencyInput,
    artifacts: Artifact[],
  ): Promise<{ artifacts: Artifact[]; success: boolean }> {
    ...
    writeFileSync(
      resolve(dir, `${input.title.toLowerCase().replace(/\s+/g, "-")}.json`),
      content,
    );
    artifacts.push({
      path: `content/financials/${input.title.toLowerCase().replace(/\s+/g, "-")}.json`,
      action: "created",
      content,
      metadata: {},
    });
  ...
  private async publishAudit(
    ...
    writeFileSync(
      resolve(dir, `${input.title.toLowerCase().replace(/\s+/g, "-")}.md`),
      content,
    );
    artifacts.push({
      path: `docs/audit/${input.title.toLowerCase().replace(/\s+/g, "-")}.md`,
      action: "created",
      content,
      metadata: {},
    });
  ...
  private async publishGovernance(
    input: TransparencyInput,
    artifacts: Artifact[],
  ): Promise<{ artifacts: Artifact[]; success: boolean }> {
    ...
    artifacts.push({
      path: `docs/governance/active/${input.title.toLowerCase().replace(/\s+/g, "-")}.md`,
      action: "created",
      content,
      metadata: {},
    });
