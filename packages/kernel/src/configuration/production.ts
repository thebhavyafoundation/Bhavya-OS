...
export class ProductionConfiguration {
  ...
  // Set config value
  set<K extends keyof ProductionConfig>(key: K, value: ProductionConfig[K]): void {
  ...
  private loadFromEnv(): void {
    ...
    if (env.LOG_LEVEL) this.config.logLevel = env.LOG_LEVEL as any;
    ...
    if (env.TOKEN_EXPIRY) this.config.auth.tokenExpiry = parseInt(env.TOKEN_EXPIRY);
    if (env.REFRESH_EXPIRY) this.config.auth.refreshExpiry = parseInt(env.REFRESH_EXPIRY);

    if (env.RATE_LIMIT_WINDOW) this.config.rateLimit.windowMs = parseInt(env.RATE_LIMIT_WINDOW);
    if (env.RATE_LIMIT_MAX) this.config.rateLimit.maxRequests = parseInt(env.RATE_LIMIT_MAX);

    if (env.MONITORING_ENABLED) this.config.monitoring.enabled = env.MONITORING_ENABLED === 'true';
    if (env.MONITORING_SAMPLE_RATE) this.config.monitoring.sampleRate = parseFloat(env.MONITORING_SAMPLE_RATE);
  }
  ...
  toSafeLog(): Record<string, unknown> {
    return {
      ...
      database: { host: this.config.database.host, port: this.config.database.port, ssl: this.config.database.ssl },
