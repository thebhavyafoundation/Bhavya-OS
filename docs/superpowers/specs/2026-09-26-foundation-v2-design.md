...
| D4  | **Photos:** user supplies hero (person over snow-peak sunrise) + quote-band panorama later; I source mission-tile photos; existing `public/photography/` JPEGs used as interim; credits = author + license only, **no place names** (also stripped from alt text). **SUPERSEDED 2026-09-26 (user feedback):** no image source/credit text is rendered anywhere — `photo-credits` layer removed; gallery (Plan C) carries no credit field. Alt-text place-name ban still stands. | User directive (updated 2026-09-26).                                                                                                    |
