...
if (content.includes('"math-1-1"')) { console.log('already has math, skipping'); process.exit(0); }
...
let idx = content.indexOf('// HELPER FUNCTIONS');
if (idx === -1) { console.error('HELPER not found'); process.exit(1); }
// Find the block comment start preceding HELPER
let insertPos = content.lastIndexOf('// \u2550', idx);
if (insertPos === -1) insertPos = idx;
// Extract up to insertPos and insert block before it
let before = content.slice(0, insertPos);
// The part from insertPos onward is the HELPER block etc.
let after = content.slice(insertPos);
// Need to ensure previous entry ends with comma. The lessonContents object ends with "  },\n};\n\n" before helper.
// Find last "  }," before helper - ensure comma.
// If before ends with "  },\n" we need to ensure comma exists. The block starts with comma-separated entries, so we need to adjust.
// The simplest: if before trimmed ends with "}," then we add our block after that, else add comma.
let trimmed = before.replace(/\s+$/, '');
let needsComma = trimmed.endsWith('}');
let blockWithComma = (needsComma ? ',' : '') + block + '\n\n';
let finalContent = before + blockWithComma + after;
fs.writeFileSync(path, finalContent, 'utf8');
console.log('done len', finalContent.length);
